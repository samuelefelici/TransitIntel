...
router.post("/stops", async (req, res) => {
  try {
    ...
    req.log.error(err, "Error creating stop");
    res.status(500).json({ error: "Internal server error" });
  }
});
...
router.delete("/stops/:id", async (req, res) => {
  try {
    ...
    req.log.error(err, "Error deleting stop");
    res.status(500).json({ error: "Internal server error" });
  }
});

router.get("/stops/:id/nearby", async (req, res) => {
  try {
    const { id } = req.params;
    ...
    req.log.error(err, "Error fetching nearby data");
    res.status(500).json({ error: "Internal server error" });
  }
});
