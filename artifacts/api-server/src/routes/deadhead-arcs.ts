...
import { depots, gtfsTrips, gtfsStopTimes, gtfsStops } from "@workspace/db/schema";
...
router.post("/deadhead-arcs/generate", asyncHandler(async (req, res) => {
  ...
  // Coppie: deposito→capolinea e ritorno; capolinea↔capolinea entro maxKm
  ...
  res.json({
    created,
    skippedExisting: pairs.length - todo.length - (truncated ? 0 : 0),
    estimated,
    truncated,
