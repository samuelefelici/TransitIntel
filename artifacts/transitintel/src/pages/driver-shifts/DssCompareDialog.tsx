...
export function DssCompareDialog({ scenarioId, savedDss, defaultLeftId, defaultRightId, onClose }: Props) {
  ...
  const rows = useMemo(() => {
    ...
    return [
      ...
      { label: "Auto aziendali (picco)", l: sL?.companyCarsMaxSimultaneous ?? sL?.companyCarsUsed, r: sR?.companyCarsMaxSimultaneous ?? sR?.companyCarsUsed, lowerIsBetter: true, unit: "" },
      { label: "Viaggi auto senza auto", l: sL?.companyCarsConflicts ?? 0, r: sR?.companyCarsConflicts ?? 0, lowerIsBetter: true, unit: "" },
      { divider: true } as any,
