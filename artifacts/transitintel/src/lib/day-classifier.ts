/**
 * CLASSIFICATORE GIORNI (copia client del modulo puro
 * api-server/src/lib/day-classifier.ts — stessa semantica, stesse chiavi).
 *
 * Serve al Calendario Aziendale per l'ANTEPRIMA LIVE: la griglia annuale si
 * ricalcola in locale sul profilo in bozza, senza dover salvare (il salvataggio
 * riscrive in massa le categorie: non è un'operazione da usare come "preview").
 * Se cambi l'albero di classificazione lato server, aggiorna anche questo file.
 *
 * Albero: 1° livello SCUOLE APERTE | SCUOLE CHIUSE | FESTIVO;
 * 2° livello scuole chiuse → INVERNALE|ESTIVO, festivo → DOMENICA(aperte|chiuse)|ROSSO;
 * 3° livello Feriale|Sabato. Precedenza: rosso > domenica > chiuse > aperte.
 */

export interface CalendarProfile {
  closedPeriods: Array<{ from: string; to: string; label?: string; kind?: "estivo" | "invernale" }>;
  /** @deprecated periodo estivo unico legacy (fallback per periodi senza kind) */
